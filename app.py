...
def length():
    ...
    conversion_result = session.pop("result", None)
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or unit_from not in available_units
            or unit_to not in available_units
        ):
            return redirect(url_for("length"))

        conversion_result = converter_unit(
            units=LENGTH, value=value, unit_from=unit_from, unit_to=unit_to
        )
        formatted_result = f"{value}{unit_from} = {conversion_result}{unit_to}"
        # add result in session
        session["result"] = formatted_result
        return redirect(url_for("length"))

    return render_template("./pages/length.html", units=available_units, result=conversion_result)
